...
export default function SessionRecap() {
  ...
  return (
    <div className="space-y-6">
      <section className="bg-white rounded-2xl border border-gray-200 shadow-sm p-5 sm:p-6 text-center space-y-2">
        <Trophy className="w-12 h-12 mx-auto text-amber-500" />
        <h2 className="text-2xl sm:text-3xl font-bold text-gray-900 leading-tight">Dobra robota!</h2>
      ...
      <section className="grid grid-cols-2 gap-3 sm:gap-4">
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-4 sm:p-5 text-center">
          ...
          <p className="text-2xl sm:text-3xl font-bold text-gray-900">{stats.durationMinutes}</p>
          <p className="text-sm text-gray-500">minut</p>
        </div>
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-4 sm:p-5 text-center">
          <Dumbbell className="w-6 h-6 mx-auto text-emerald-600 mb-2" />
          <p className="text-2xl sm:text-3xl font-bold text-gray-900 break-words">{stats.totalVolume.toLocaleString('pl-PL')}</p>
          <p className="text-sm text-gray-500">kg wolumenu</p>
        </div>
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-4 sm:p-5 text-center">
          <Layers className="w-6 h-6 mx-auto text-violet-600 mb-2" />
          <p className="text-2xl sm:text-3xl font-bold text-gray-900">{stats.totalSets}</p>
          <p className="text-sm text-gray-500">serii</p>
        </div>
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-4 sm:p-5 text-center">
          <Dumbbell className="w-6 h-6 mx-auto text-orange-600 mb-2" />
          <p className="text-2xl sm:text-3xl font-bold text-gray-900">{stats.totalExercises}</p>
          <p className="text-sm text-gray-500">ćwiczeń</p>
        </div>
      </section>

      {stats.prs.length > 0 && (
        <section className="bg-amber-50 rounded-2xl border border-amber-200 shadow-sm p-4 sm:p-5 space-y-3">
          ...
          {stats.prs.map((pr, i) => (
            <div key={`${pr.exerciseId}-${pr.type}-${i}`} className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between text-sm">
              <span className="font-medium text-amber-900 break-words">{pr.exerciseName}</span>
